"""
Combination of GAN planner and RL planner
Run on RLLIB, modified from stable baseline's myenv.py
Gym environment for 1D reinforcement learning problem inspired by RAND corporations report:
...
"""
MISSION_CONDITIONS = ['w1', 'w2', 'w3']
MISSION_RATIO = [0, 0, 1]

GENERATOR_NAME = 'my_generator_model_2000'
CODING_SIZE = 20

PLANNER = 'GAN+RL'  # 'RL' or 'GAN+RL'
DT_COEF = 1
STEP_REWARD_COEF = 0.1
PLAN_MARGIN = 0.5
...
class Fighter:
    ...
    FIGHTER_ALLOWABLE_NEGATIVE_INGRESS = -100  # km
...
class Jammer:
    JAMMER_ALLOWABLE_NEGATIVE_INGRESS = -100  # km
...
class MyEnv(gym.Env):
    ...
    SPACE_Y = SPACE_X  # km
    SPACE_OFFSET = -100.  # km, negative size of battle space
    ...
    def __init__(self, env_config):
        ...
        self.generator = load_model(GENERATOR_NAME)  # GAN planner
        ...
        self.dt = 1 / self.fighter.speed * DT_COEF  # simulation step
    ...
    def reset_render_fighter_plan(self):
        # Render coordinate
        self.fighter.screen_x = self.fighter.plan * self.to_screen_x - self.to_screen_offset
        self.fighter.screen_y = self.space_y / 2.1 * self.to_screen_x

        # Fighter circle
        width = self.fighter.radius
        height = self.fighter.radius
        self.fighter.plan_surface = pygame.Surface((width, height))
        pygame.draw.circle(surface=self.fighter.plan_surface,
                           center=(self.fighter.radius / 2, self.fighter.radius / 2),
                           color=self.fighter.color, radius=self.fighter.radius / 2)

    def reset_render_jammer_plan(self):
        # Render coordinate
        self.jammer.screen_x = self.jammer.plan * self.to_screen_x - self.to_screen_offset
        self.jammer.screen_y = self.space_y / 1.9 * self.to_screen_x

        # Jammer circle
        width = self.jammer.radius
        height = self.jammer.radius
        self.jammer.plan_surface = pygame.Surface((width, height))
        pygame.draw.circle(surface=self.jammer.plan_surface,
                           center=(self.jammer.radius / 2, self.jammer.radius / 2),
                           color=self.jammer.color, radius=self.jammer.radius / 2)
    ...
    def reset_gan_plan(self):
        # Normalize condition
        gan1 = self.fighter.firing_range / self.fighter.max_firing_range
        gan2 = self.sam.offset / self.sam.max_offset
        gan3 = self.sam.firing_range / self.sam.max_firing_range
        gan4 = self.sam.jammed_firing_range / (self.sam.max_firing_range * self.jammer.jam_effectiveness)
        gan_input = [gan1, gan2, gan3, gan4]
        gan_input = tf.expand_dims(tf.cast(gan_input, tf.float32), axis=0)

        z = np.random.randn(1, CODING_SIZE)

        plan = self.generator([z, gan_input])
        self.fighter.plan = plan[0, 0].numpy() * self.sam.max_offset
        self.jammer.plan = plan[0, 1].numpy() * self.sam.max_offset
    ...
    def reset(self):
        ...
        self.reset_render_fighter_plan()
        self.reset_render_jammer_plan()
    ...
    def render_fighter_plan(self):
        # transform coordinate from battle space to screen
        self.fighter.screen_x = self.fighter.plan * self.to_screen_x - self.to_screen_offset

        # draw fighter_plan
        screen_x = self.fighter.screen_x - self.fighter.radius / 2
        screen_y = self.fighter.screen_y - self.fighter.radius / 2
        self.screen.blit(self.fighter.plan_surface, (screen_x, screen_y))
    ...
    def render_jammer_plan(self):
        # transform coordinate from battle space to screen
        self.jammer.screen_x = self.jammer.plan * self.to_screen_x - self.to_screen_offset

        # draw jammer
        screen_x = self.jammer.screen_x - self.jammer.radius / 2
        screen_y = self.jammer.screen_y - self.jammer.radius / 2
        self.screen.blit(self.jammer.plan_surface, (screen_x, screen_y))
    ...
    def render(self, mode='human'):
        if mode == 'human':
            ...
            if self.jammer.alive > 0.5:
                self.render_jammer()
                self.render_jammer_plan()
            if self.fighter.alive > 0.5:
                self.render_fighter()
                self.render_fighter_plan()
    ...
    def get_reward_2(self, done):
        """
        Reward for RL Planner only architecture
        """
        reward = 0
    ...
    def get_reward(self, done):
        if PLANNER == 'GAN+RL':
            reward = self.get_reward_1(done)  # Reward for GAN + RL planner
        elif PLANNER == 'GAN':
            reward = self.get_reward_2(done)  # Reward for no GAN planner only
        else:
            raise Exception('Specufy proper planner!')
